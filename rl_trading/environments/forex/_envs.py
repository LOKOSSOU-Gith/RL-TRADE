...
class ForexEnv(gym.Env, ABC):
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        '''
        # S'assurer que l'index ne dépasse pas la taille du DataFrame
        current_index = min(self._t, len(self) - 1)
        if current_index < 0:
            current_index = 0
            
        return np.concatenate([
            self._features_df.iloc[current_index].values,
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        # Vérification de sécurité pour éviter l'erreur d'index
...
class ForexEnvBasic(ForexEnv):
    ...
    def _reset(self, start_t: int=0):
        ...
        # S'assurer que l'index ne dépasse pas la taille du DataFrame
        reset_index = min(start_t, len(self) - 1)
        if reset_index < 0:
            reset_index = 0
            
        self._last_trade_price = self._target_prices_df.loc[reset_index, '<OPEN>'] 
        self._order_size = 0
    ...
    def _step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        # Vérification de sécurité avant d'accéder aux données
        if current_index >= len(self._target_prices_df) - 1:
            self._done = True
            return self._get_observation(), 0.0, True, {}
            
        open_price = self._target_prices_df.loc[current_index, '<OPEN>']
        close_price = self._target_prices_df.loc[current_index, '<CLOSE>']

        if self._t > 0 and current_index > 0:
            prev_close_price = self._target_prices_df.loc[current_index - 1, '<CLOSE>']
