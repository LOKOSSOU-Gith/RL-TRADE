...
def load_raw_forex_data(
    ...
    elif data_source == ForexDataSource.TWELVEDATA:
        return _load_raw_twelvedata_forex_data(pairs)
...
def _load_raw_twelvedata_forex_data(
    pairs: List[str]
) -> Dict[str, pd.DataFrame]:
    '''
    Loads original forex data from Twelve Data API.
    ...
    Returns:
        Original dataframes for each pair
    '''
    from .twelve_data_source import get_historical_data
    
    twelvedata_data = {}

    for pair in pairs:
        try:
            # Récupérer les données depuis Twelve Data
            pair_df = get_historical_data(symbol=pair)
            twelvedata_data[pair] = pair_df
        except Exception as e:
            print(f"Erreur lors du chargement des données pour {pair}: {e}")
            # Créer un DataFrame vide en cas d'erreur
            twelvedata_data[pair] = pd.DataFrame(columns=['<DT>', '<OPEN>', '<HIGH>', '<LOW>', '<CLOSE>'])
